...
from transtab.trainer_utils import (
    TrainDataset, get_parameter_names, get_scheduler
)
from tabtransformer.trainer_utils import TabTransformerCollatorForSL
...
class Trainer:
    def __init__(self,
        ...
        **kwargs,
        ...
        assert collate_fn is not None, f"Collate function of 'None' is not allowed"
    ...
    def train(self):
        ...
        if args['warmup_ratio'] is not None or args['warmup_steps'] is not None:
            ...
            print(f'set warmup training in initial {num_train_steps} steps')
        ...
        for epoch in range(args['num_epoch']):
            ...
            print('epoch: {}, train loss: {:.4f}, lr: {:.6f}, spent: {:.1f} secs'.format(
                epoch, train_loss_all/ite, self.optimizer.param_groups[0]['lr'], time.time()-start_time
            ))
